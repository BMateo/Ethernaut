...
/**
    When the sweepToken function is called, the DET tokens are transfered out of the vault contract and that isnt supossed to happen
    The delegate transfer function will take as msg.sender the vault, that has balance of DET, and transfer it out.
    To solve this problem we need to raise an alert when the delegateTransfer function its called by the vault contract.
    This can be done analizing the msg.data that the notify function in the forta contract receives
 */
